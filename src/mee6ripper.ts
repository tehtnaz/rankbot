...
async function GetPageLevels(serverId: string, pageNum: number) {
    ...
    const temp = await axios.get(`https://mee6.xyz/api/plugins/levels/leaderboard/${serverId}?limit=1000&page=${pageNum}`);
...
async function GetRoleRewards(serverId: string): Promise<(Required<RoleReward>)[]> {
    ...
    return (data.role_rewards as any[]).map((role_reward) =>{return {role_id: role_reward.role.id, level: role_reward.rank, name: role_reward.role.name}});
}
...
async function ParseLevelsIntoRanges(serverId: string, maxPageNum: number) {
    ...
    while(consumedAmount < levels.length){
        ...
        consumed = levels.filter((val) => {return (val.level === xpMaxRange)});
...
async function TransferAllDataIntoDatabase() {
    ...
    const sameOrNewQuestion = readLine.question("Would you like to transfer all the data to a new server or to the same server as before? (same/new): ");
    const server_id = sameOrNewQuestion === "same" ? allTempLevels[0].server_id : readLine.questionInt("What is the ID of the new server?: ").toString();
    ...
    for(const roleReward of allRoleRewards){
        const newId = sameOrNewQuestion === "same" ? roleReward.role_id : readLine.questionInt(`roleReward named "${roleReward.name}" with level requirement "${roleReward.level}" has a stored role id of "${roleReward.role_id}"\nWhat would you like the new ID of it to be?: `);
    ...
    for(const personXp of allTempLevels){
        promiseArray.push(prodPersonXp.create({user_id: personXp.user_id, server_id: personXp.server_id, counted_msg: personXp.counted_msg, xp: personXp.xp, date: 0, lvl: personXp.lvl, lvlxp: personXp.lvlxp}));
    }
    Promise.all(promiseArray).then(() =>{console.log("Finished all syncing. Enjoy your new levels!");})
